...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_channel":
        if await check_channel_subscription(context.bot, query.from_user.id):
            await query.answer("تم التأكد من الاشتراك ✅", show_alert=True)
            await show_main_menu(update, query.from_user.id, context)
        else:
